...
class HTZInputDBManager(object):
    def __init__(self):
        ...
        self.__alphabet_list = ["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z"]
...
# ============================================================================================================================
# ============================================================================================================================
    def set_connection_info(self, host_address:str, port_num:int, username:str, password:str, db_name:str):
        """
...
# ============================================================================================================================
# ============================================================================================================================
    def overwrite_update_req_table_into_complete_table(self, base_table_name:str):
        """
        HTZ 입력 DB에서 HTZ 분석 완료 DB로 옮기는 작업을 하는 함수

        리턴값:
            included_target_info1_list, excluded_target_info1_list
        """
        included_target_info1_list = []
        excluded_target_info1_list = []

        if(self.connect_called == False):
            print(" ** Error) IRSS DB Manager / overwrite_update_req_table_into_complete_table() : Called without connect()")
            return included_target_info1_list, excluded_target_info1_list
        ...
        # 1) 업데이트 요청 DB에서 Info1의 목록(=대상 [기지국 식별자]_[번호]_[각도])을 획득
        sql_query_str = "SELECT * FROM " + update_req_table_name
        try:
            self.cur.execute(sql_query_str)
            self.conn.commit()
        ...
        for table_row in query_result_rows:
            ...
            if(table_row[self.column_index_ics_value] == None):
                ...
                excluded_target_info1_list.append(info1_rds_pms_no)
            else:                
                included_target_info1_list.append(info1_rds_pms_no)

        # 2) 완료 DB에 Info1과 동일 항목이 있을 경우 모두 삭제 (기존 결과 삭제)
        #    INFO1 : [기지국번호]_[번호]_[각도]
        for target_info1 in included_target_info1_list:
...
# ============================================================================================================================
    def insert_kca_json_entry_into_update_req_table_v2(self, base_table_name:str, kca_json_entry, Cus_cd:str) -> bool:                
        ...
        Address_str = str(kca_json_entry.get("RDS_TRS_ADR"))  
        if len(Address_str) >= 50:                              # RDS_TRS_ADR 크기가 길어서 50 이상이면 자르기
        ...
        if(len(azimuth_str_array) >= 2) : 
            for idx, azimuth_str in enumerate(azimuth_str_array):
                ...
                # Info1 : [기지국번호]_[번호]_[수평각도]
                #  - DB 구조상 Primary Key이므로 중복 불가
                #  - 가끔 2개 이상 섹터가 수평각도가 일치하는 경우가 있어 Index를 추가로 붙여주어 중복 방지
