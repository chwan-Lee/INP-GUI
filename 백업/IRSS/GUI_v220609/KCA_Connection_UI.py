...
class KCA_Connection_UI(object):
    def __init__(self):
        ...
        self.root = Tk()
        self.root.title("KCA DB 관리 프로그램")         # 창 이름
    ...
    def __IrssDBConnection_Setup(self) -> bool:
        result = self.__updateIrssDBConnInputFromUI()
        if(result == False):
            return False          

        self.irss_db_manager.set_connection_info(self.result_hostname, self.result_portnumber, self.result_username, self.result_password)
        if(self.irss_db_manager.connect() == False):
            return False

    def __KcaOriginalDBConnection_Setup(self) -> bool:
        result = self.__updateKcaConnParameterFromUI()
        if(result == 0):
            return False       

        self.kca_originaldatadb_manager.set_connection_info(self.result_hostname, self.result_portnumber, self.result_username, self.result_password)
        ...
        for service_provider_name in self.res_serv_prov_list:                   # 서비스 제공자
            ...
            for service_type in self.res_serv_type_list:                        # 서비스 유형 (2G, ...)
                ...
                if(num_rows_in_table > 0):                    
                    ...
                    self.__write_log_into_file("실행 : " + batch_file_name)
                    os.system(batch_file_name)

                    # 업데이트 필요 DB --> 완료 DB로 이동
                    self.__write_log_into_file("실행완료 : " + batch_file_name + ". DB 이동 시작")
                    result = self.irss_db_manager.overwrite_update_req_table_into_complete_table(base_table_name)
                    self.__write_log_into_file("실행완료 : " + batch_file_name + ". DB 이동 완료")
                else:
                    pass
    ...
    def __onTimerTest(self):
        ...
        if(self.timer_is_on):
            ...
            self.timer = threading.Timer(60, self.__onTimerTest)
            self.timer.start()
            message_str = "자동 실행 완료 : " + str(now) + ". 60초 후 재실행"
    ...
    def __make_kca_connection_ui(self):
        ...
        self.btnTimerTest.place(x=base_x + button_width_large + 20, y=new_y, width=button_width_large, height=button_height)
        ...
        self.btnDBTableCreate.place(x=base_x, y=new_y, width=button_width_large, height=button_height)        
        ...
        self.btnDBTableEmpty.place(x=base_x+(button_width_large+button_x_margin), y=new_y, width=button_width_large, height=button_height)
